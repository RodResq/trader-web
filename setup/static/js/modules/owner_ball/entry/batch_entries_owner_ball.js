...
function toggleChecklist() {
    ...
    const table = document.getElementById('ownerBallMarketsTable')
...
async function verificarCiclo() {
    ...
    const url = `api/v1/owner_ball/cycle/check-date/${dataFormatada}`
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error('Erro ao recuperar ciclo');
        }
        ...
        if (data.success) {
            if (data.exists) {
                ...
                saldoDisponivel.textContent = parseFloat(data.data.current_balance).toFixed(2);
                ...
                if (!valorEntradaInput.value) {
                    const valorSugerido = Math.min(
                        data.data.available_value * 0.1,
                        data.data.available_value
                    ).toFixed(2);
                ...
                showNotification('Data nao existe no ciclo onwver ball. Por favor cadastre um novo ciclo', 'warning');
            }
        } else {
        ...
        showNotification('Erro ao recuperar dados do cyclo', 'danger');
    }
