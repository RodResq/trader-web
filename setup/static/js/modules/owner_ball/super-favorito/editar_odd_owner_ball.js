...
export function setupEditarModalOwnerBall() {
    const modal = obterInstanciaModal();

    const editarOddBtnsOwnerBall = document.querySelectorAll('.edit-odd-btn-owner-ball');
    const confirmBtn = document.getElementById('confirmarEdicaoOddBtn');
    const novaOddInput = document.getElementById('novaOddInput');

    if (!modal || !editarOddBtnsOwnerBall.length || !confirmBtn || !novaOddInput) return;
    ...
    editarOddBtnsOwnerBall.forEach(button => {
...
function obterInstanciaModal() {
    const modalElement = document.getElementById('editarOddModal');
